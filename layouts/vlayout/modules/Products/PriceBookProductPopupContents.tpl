	...
	{if $LISTVIEW_ENTRIES_COUNT eq '0'}
		<div class="row-fluid">
			{*<div class="emptyRecordsDiv">{vtranslate('LBL_EQ_ZERO', $MODULE_NAME)} {vtranslate($MODULE_NAME, $MODULE_NAME)} {vtranslate('LBL_FOUND', $MODULE_NAME)}.</div>*}
                        ...
                        {* Frodis.ru - начало *}
                        {* Для нормального перевода в списке контактов (контрагентов и тд) когда нет записей*}
                        {* Также нужно изменить в файлах перевода метку "LBL_NOT_FOUND" *}
                        <div class="emptyRecordsDiv">{vtranslate('LBL_NOT_FOUND')}.</div>
                        {* Frodis.ru - конец *}
		</div>
	{/if}
</div>
