										...
										{else}
											...
											{/if}>
												{*<input type="checkbox" class="alignTop" {if empty($RECORD_ID)} checked="true" {/if} name="permissions[{$TABID}][actions][{$ACTIONID}]" {if $RECORD_MODEL->hasModuleActionPermission($PROFILE_MODULE, $ACTIONID)}checked="true"{/if}> {$ACTION_MODEL->getName()}</td>*}
                                                                                                {* Frodis.ru - Начало *}
                                                                                                <input type="checkbox" class="alignTop" {if empty($RECORD_ID)} checked="true" {/if} name="permissions[{$TABID}][actions][{$ACTIONID}]" {if $RECORD_MODEL->hasModuleActionPermission($PROFILE_MODULE, $ACTIONID)}checked="true"{/if}> {vtranslate($ACTION_MODEL->getName(), $MODULE_NAME)}</td>
                                                                                                {* Frodis.ru - Конец *}
