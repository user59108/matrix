...
class PBXManager_PBXManager_Connector {
    ...
    public function handleHangupCall($details) {
        ...
        switch ($hangupcause) {
            ...
            case 'Normal Clearing':
                ...
                // Frodis.ru - Начало
                // Время завершения (EndTime) разговора при удачном звонке (когда на звонок ответили, поговорили и положили трубку) не приходит,
                // поэтому получим его из другого параметра (dataReceived)                
                if (!$details->get('EndTime') && $details->get('dateReceived')) {
                    $endtime = DateTime::createFromFormat('D M d G:i:s e Y', $details->get('dateReceived'));
                    $endtime->setTimezone(new DateTimeZone('UTC'));
                    $details->set('EndTime', $endtime->format('Y-m-d G:i:s'));    
                }
                // Так же не приходит длительность звонка (Duration), поэтому определяем вручную 
                if (!$details->get('Duration') && $recordModel->get('starttime')) {
                    $starttime = new DateTime($recordModel->get('starttime'), new DateTimeZone('UTC'));
                    $duration = $endtime->diff($starttime);
                    $details->set('Duration', $duration->format('%s'));
                }            
                // Frodis.ru - Конец
    ...
    public function respondToIncomingCall($details) {
        ...
        // Frodis.ru - Начало
        $smowner = PBXManager_Record_Model::getInstanceBySourceUUID($details->get('callUUID'));
        $smownerid = $smowner->get('user');
        // Frodis.ru- Конец
        ...
        if (self::$NUMBERS) {
            ...
            foreach (self::$NUMBERS as $userId => $number) {
                ...
                // Frodis.ru - Начало
                if ($smownerid AND ($smownerid != $userId)) {
                    continue;
                }
                // Frodis.ru - Конец
