...
class PBXManager_Record_Model extends Vtiger_Record_Model{
    ...
    public static function lookUpRelatedWithNumber($from){
        ...
        if($db->num_rows($result)){
            ...
            // Frodis.ru - Начало
            // Добавим в список полей, данные по которым хотим получить, поле ответственного smownerid
            $contact = $db->pquery('SELECT label,setype,smownerid FROM '.self::entitytableName.' WHERE crmid=? AND deleted=0', array($crmid));
            // Frodis.ru - Конец
            if($db->num_rows($result)){
                $data['id'] = $crmid;
                ...
                // Frodis.ru - Начало
                // Получим id ответственного и запишем в возвращаемый массив 
                $data['smownerid'] = $db->query_result($contact, 0, 'smownerid');
                // Frodis.ru - Конец
                return $data;
            }
            else
