	...
	public static function getPackagesList() {
		...
		for ($i = 0; $i < $numOfRows; $i++) {
			...
			if (!$moduleExists) {
                ...
                // Frodis.ru - Начало
                foreach ($packagesList as $key=>$values){
                    $packagesList[$key]['description'] = vtranslate($values['description']);
                    $packagesList[$key]['label'] = vtranslate($values['label']);
                    foreach ($values['modules'] as $k=>$value) {
                        $packagesList[$key]['modules'][$k] = vtranslate($k);             
                    }
                }
                // Frodis.ru - Конец                
